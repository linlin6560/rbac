    ...
    <el-card>
        <el-form :inline="true" :model="queryParams" ref="queryRef" v-show="showSearch">
            ...
            <el-form-item label="菜单状态" prop="menuStatus">
                <el-select placeholder="请选择部门状态" v-model="queryParams.menuStatus">
        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermission="['base:menu:add']">
                    新增
                </el-button>
            </el-col>
            <el-col :span="1.5">
            ...
            <TipHover v-model:showSearch="showSearch" @queryTable="getMenuList" :columns="columns" />
        </el-row>
        <el-table border stripe :header-cell-style="{ background: '#eef1f6', color: '#606266' }" v-loading="loading"
            :data="menuList" row-key="id" :tree-props="{ children: 'children' }" v-if="refreshTable"
            ...
            <el-table-column label="菜单状态" prop="menuStatus" v-if="columns[5].visible">
                <template #default="scope">
                    <el-tag v-if="scope.row.menuStatus === 2" type="success">正常</el-tag>
                    <el-tag v-else-if="scope.row.menuStatus === 1" type="danger">停用</el-tag>
                </template>
            </el-table-column>
            ...
            <el-table-column label="更多操作" class-name="small-padding fixed-width">
                <template #default="scope">
                    <el-button type="primary" link icon="Edit" @click="handleUpdate(scope.row.id)" v-hasPermission="['base:menu:edit']">修改
                    </el-button>
                    <el-button type="danger" link icon="Delete" @click="handleDelete(scope.row.id)" v-hasPermission="['base:menu:delete']">删除
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
            ...
            <el-form ref="menuRef" :model="form" :rules="rules" label-width="80px">
                ...
                <el-form-item label="上级菜单">
                    <el-tree-select v-model="form.parentId" :data="addList" :props="{ value: 'id', label: 'menuName' }"
                        value-key="id" placeholder="选择上级菜单" check-strictly />
                </el-form-item>
                ...
                <el-form-item v-if="form.menuType == 3" label="权限标识" prop="value">
                    <el-input v-model="form.value" placeholder="请输入权限标识" maxlength="50" />
                </el-form-item>
                <el-form-item label="菜单状态" prop="menuStatus">
